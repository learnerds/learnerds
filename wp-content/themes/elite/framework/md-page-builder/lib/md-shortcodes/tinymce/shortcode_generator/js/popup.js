jQuery(document).ready(function($){
		...
		*/
		...
		$('#options-'+name+' [data-shortcode="md_accordion"]').each(function(){
			if($(this).find('textarea').val()){
				code_wrapped += '[md_accordion title="'+raw_encode($(this).find('input').val())+'"]'+raw_encode($(this).find('textarea').val())+'[/md_accordion]';
				$close_sh = true;
			}
		});
		...
		$('#options-'+name+' [data-shortcode="md_toggle"]').each(function(){
			if($(this).find('textarea').val()){
				code_wrapped += '[md_toggle title="'+raw_encode($(this).find('input').val())+'"]'+raw_encode($(this).find('textarea').val())+'[/md_toggle]';
				$close_sh = true;
			}
		});
		...
		$('#options-'+name+' [data-shortcode="md_tour"]').each(function(){
			if($(this).find('textarea').val()){
				code_wrapped += '[md_tour title="'+raw_encode($(this).find('input').val())+'"]'+raw_encode($(this).find('textarea').val())+'[/md_tour]';
				$close_sh = true;
			}
		});
		...
		$('#options-'+name+' [data-shortcode="md_tab"]').each(function(){
			if($(this).find('textarea').val()){
				code_wrapped += '[md_tab title="'+raw_encode($(this).find('input').val())+'"]'+raw_encode($(this).find('textarea').val())+'[/md_tab]';
				$close_sh = true;
			}
		});
    ...
    $('#add-shortcode').click(function(e){
		...
		ed.setContent( $('#shortcode-storage-o').html() );
	...
	function detectPageBuilder(){
		if($('.wpb-element-edit-modal').length){
			...
			if($pb_sh){
				...
				if($pb_sh == 'md_accordions' || $pb_sh == 'md_toggles' || $pb_sh == 'md_tours' || $pb_sh == 'md_tabs' || $pb_sh == 'md_carousel'){
    				...
    				for($x = 0; $x < $('#generate-sh').find('code').length; $x++){
    					...
    					$('#options-'+$pb_sh).find('.shortcode-dynamic-item').eq($x).find('input').val(raw_decode($('#generate-sh').find('code').eq($x).attr('title')));
    					$('#options-'+$pb_sh).find('.shortcode-dynamic-item').eq($x).find('textarea').val(raw_decode($('#generate-sh').find('code').eq($x).html()));
					}
				}
			}
		}
