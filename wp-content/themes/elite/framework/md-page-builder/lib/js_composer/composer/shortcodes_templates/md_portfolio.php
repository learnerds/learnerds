...
if($show_filter && !$carousel && $style!= 'masonry'){
	...
	if ($category){
		$category_filter = explode(',', $category);
	}
	...
	$output .= '<div class="md-portfolio-filter '.$filter_style.' '.$style.'"><span class="current">All</span><span class="lbl">Filter:</span>';
		...
		foreach($categories as $category):
			...
			if(!$category_filter || $category_filter && in_array($category->slug, $category_filter))
...
if($style == 'default' && !$carousel){
	...
	$output .= '<div'.$class.$id.$css_animation_delay.'>';
		...
		if($carousel){
			...
			$output .= '<div class="md-carousel '.$carousel_style.'" data-items="'.$items_cols.'" data-navigation="'.$carousel_navigation.'" data-pagination="'.$carousel_pagination.'" data-autoplay="'.$carousel_autoplay.'">';
		}
